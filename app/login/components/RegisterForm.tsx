...
export default function RegisterForm({ isActive, onSuccessfulRegister }: { isActive: boolean, onSuccessfulRegister: () => void }) {
  ...
  return (
    <div className={`w-full md:w-1/2 h-full flex flex-col p-5 md:p-6 transition-all duration-500 overflow-y-auto custom-scrollbar ${isActive ? 'block' : 'hidden md:flex'}`}>
      
      {registerView === "details" && (
        <div className="animate-in fade-in zoom-in-95 duration-300 w-full max-w-xs mx-auto my-auto pb-4 md:pb-0">
          <div className="text-center mb-4">
            <div className="w-10 h-10 bg-purple-50 rounded-full flex items-center justify-center mx-auto mb-2 md:hidden">
              <Building2 className="text-[#a07be1] pointer-events-none" size={18} />
            </div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight">Create Account</h1>
            <p className="text-[11px] text-slate-500 mt-0.5 font-medium">Register your laboratory environment</p>
          </div>
          ...
          <form onSubmit={handleRegisterDetails} className="space-y-3">
            <div className="relative">
              <Building2 className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={14} />
            ...
            <div className="relative">
              <Phone className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={14} />
            ...
            <button disabled={isLoading} type="submit" className={`${buttonBaseClass} mt-2`}>
              {isLoading ? <Loader2 className="animate-spin" size={14} /> : "Continue"}
            </button>
          </form>
        </div>
      )}
      ...
      {registerView === "verify" && (
        <div className="animate-in fade-in slide-in-from-right-4 duration-300 w-full max-w-xs mx-auto my-auto">
          <div className="text-center mb-4">
            <h1 className="text-xl font-black text-slate-800 tracking-tight">Verify Email</h1>
            <p className="text-[11px] text-slate-500 mt-0.5">Sent to <span className="font-bold text-[#a07be1] truncate block max-w-xs mx-auto">{regEmail}</span></p>
          </div>
          ...
          <form onSubmit={handleVerifyRegisterOtp} className="space-y-3">
            <div className="relative">
              ...
              <input type="text" required value={regOtp} onChange={(e) => setRegOtp(e.target.value)} className={`${inputBaseClass} font-mono tracking-[0.2em] text-center text-[14px]`} placeholder="000000" />
            </div>
            
            <button disabled={isLoading} type="submit" className={buttonBaseClass}>
              {isLoading ? <Loader2 className="animate-spin" size={14} /> : "Verify Account"}
            </button>

            <button type="button" onClick={() => { setRegisterView("details"); setError(""); setMessage(""); }} className="w-full h-8 text-[11px] font-bold text-slate-500 hover:text-slate-800 flex items-center justify-center gap-1 transition-colors mt-2 outline-none focus:outline-none bg-slate-100 rounded-md">
        ...
        <div className="animate-in fade-in slide-in-from-right-4 duration-300 w-full max-w-xs mx-auto my-auto">
          <div className="text-center mb-4">
            <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-2">
              <CheckCircle2 className="text-green-600 pointer-events-none" size={14} />
            </div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight">Secure Account</h1>
            <p className="text-[11px] text-slate-500 mt-0.5 font-medium">Create your permanent password.</p>
          </div>

          {error && <div className="p-1.5 mb-2.5 bg-red-50 text-red-600 text-[10px] font-bold rounded-md border border-red-100 text-center">{error}</div>}
          {message && <div className="p-1.5 mb-2.5 bg-emerald-50 text-emerald-600 text-[10px] font-bold rounded-md border border-emerald-100 text-center">{message}</div>}

          <form onSubmit={handleSetRegisterPassword} className="space-y-3">
            <div className="relative">
            ...
            <div className="relative">
              ...
              {/* 🚨 FIX: Corrected onChange to use setRegConfirmPassword */}
            ...
            <button disabled={isLoading} type="submit" className={`${buttonBaseClass} mt-2`}>
              {isLoading ? <Loader2 className="animate-spin" size={14} /> : "Complete Registration"}
            </button>
          </form>
        </div>
      )}
