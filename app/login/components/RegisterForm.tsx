...
export default function RegisterForm({ isActive, onSuccessfulRegister }: { isActive: boolean, onSuccessfulRegister: () => void }) {
  ...
  return (
    <div className={`w-full md:w-1/2 h-full flex flex-col justify-center p-5 md:p-8 transition-all duration-500 overflow-y-auto custom-scrollbar ${isActive ? 'block' : 'hidden md:flex'}`}>
      
      {registerView === "details" && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 w-full max-w-sm mx-auto my-auto pb-4 md:pb-0">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-slate-500 mt-1 font-medium">Register your laboratory environment</p>
          </div>
      ...
      {registerView === "verify" && (
        <div className="animate-in fade-in slide-in-from-right-4 duration-300 w-full max-w-sm mx-auto my-auto">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-slate-500 mt-1">Sent to <span className="font-bold text-[#a07be1] truncate block max-w-xs mx-auto">{regEmail}</span></p>
          </div>
          ...
          <form onSubmit={handleVerifyRegisterOtp} className="space-y-4">
            <div className="relative">
              ...
              <input type="text" required value={regOtp} onChange={(e) => setRegOtp(e.target.value)} className={`${inputBaseClass} font-mono tracking-[0.2em] text-center text-base`} placeholder="000000" />
            </div>
            
            <button disabled={isLoading} type="submit" className={buttonBaseClass}>
              {isLoading ? <Loader2 className="animate-spin" size={16} /> : "Verify Account"}
            </button>

            <button type="button" onClick={() => { setRegisterView("details"); setError(""); setMessage(""); }} className="w-full h-10 text-xs font-bold text-slate-500 hover:text-slate-800 flex items-center justify-center gap-1.5 transition-colors mt-3 outline-none focus:outline-none bg-slate-100 rounded-lg">
        ...
        <div className="animate-in fade-in slide-in-from-right-4 duration-300 w-full max-w-sm mx-auto my-auto">
          <div className="text-center mb-6">
            <div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-3">
              <CheckCircle2 className="text-green-600 pointer-events-none" size={20} />
            </div>
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Secure Account</h1>
            <p className="text-sm text-slate-500 mt-1 font-medium">Create your permanent password.</p>
          </div>
          ...
          <form onSubmit={handleSetRegisterPassword} className="space-y-4">
            ...
            <button disabled={isLoading} type="submit" className={buttonBaseClass}>
              {isLoading ? <Loader2 className="animate-spin" size={16} /> : "Complete Registration"}
            </button>
          </form>
        </div>
      )}
