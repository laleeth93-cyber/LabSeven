// --- BLOCK app/parameters/add/page.tsx OPEN ---
"use client";
...
import { createParameter, generateNextParameterCode } from '@/app/actions/parameters';
...
export default function AddParameterPage() {
  ...
  useEffect(() => {
    async function loadInitialData() {
        ...
        const resCode = await generateNextParameterCode();
        if (resCode) setFormData(prev => ({ ...prev, code: resCode }));
        setIsGeneratingCode(false);

        const [resOps, resMulti, resUom, resMethod] = await Promise.all([
            getMasterData('operator'), getMasterData('multivalue'), getMasterData('uom'), getMasterData('method')
        ]);
        
        if (resOps.success) setOperators(resOps.data);
        if (resMulti.success) setMultivalueSets(resMulti.data);
        if (resUom.success) setUomList(resUom.data);
        if (resMethod.success) setMethodsList(resMethod.data);
    }
    loadInitialData();
  }, []);

  const [ranges, setRanges] = useState<any[]>([
    { gender: 'Both', minAge: 0, maxAge: 100, minAgeUnit: 'Years', maxAgeUnit: 'Years', normalOperator: '', lowRange: '', highRange: '', normalValue: [], abnormalValue: [], normalRange: '', criticalOperator: '', criticalLow: '', criticalHigh: '', criticalValue: [] }
  ]);
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      const res = await createParameter(payload);
      if(res.success) {
    ...
    <div className="h-screen w-full bg-[#f1f5f9] flex flex-col overflow-hidden font-sans relative">
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-20">
          <div className="flex items-center gap-4">
            <button onClick={() => router.push('/tests?tab=Parameters')} className="p-2 bg-slate-100 rounded-full text-slate-600 hover:text-[#9575cd] hover:bg-slate-200 transition-colors"><ArrowLeft size={18} /></button>
            <h1 className="text-xl font-bold text-slate-800 tracking-tight">Add Parameter</h1>
          </div>
          <button onClick={handleSave} disabled={isPending} className="px-6 py-2 bg-[#9575cd] text-white text-xs font-bold rounded-full shadow-md hover:bg-[#7e57c2] active:scale-95 transition-all flex items-center gap-2">
              {isPending ? <Loader2 size={16} className="animate-spin"/> : <Save size={16}/>} SAVE PARAMETER
          </button>
      </header>
                   ...
                   <div className="grid grid-cols-12 gap-6 w-full">
                      ...
                      <div className="col-span-12 md:col-span-2">
                          ...
                          <div className="relative">
                              <input type="text" disabled className={`${inputClass} bg-slate-100 text-slate-500 border-slate-200 cursor-not-allowed`} value={formData.code} placeholder={isGeneratingCode ? "Generating..." : "PRAXXXXX"} />
                              <div className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400">
                                  {isGeneratingCode ? <Loader2 size={12} className="animate-spin"/> : <Lock size={12}/>}
                              </div>
                          </div>
                      </div>

                      <div className="col-span-12 md:col-span-4"><label className={labelClass}><span className="text-red-500">*</span> Name</label><input type="text" value={formData.name} onChange={(e) => handleChange('name', e.target.value)} className={inputClass} placeholder="Name" autoFocus /></div>
...
// --- BLOCK app/parameters/add/page.tsx CLOSE ---